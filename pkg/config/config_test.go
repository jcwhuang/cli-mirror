...
func TestDynamicEnvLoad(t *testing.T) {
	t.Run("loads and validates dynamic env config", func(t *testing.T) {
		...
		assert.Equal(t, 2, len(config.DynamicEnv.Vault.StructuredEnvVars))

		// Verify first env var
		assert.Equal(t, "some-id", config.DynamicEnv.Vault.StructuredEnvVars[0].RemoteName)
		assert.Equal(t, "SOME_SECRET", config.DynamicEnv.Vault.StructuredEnvVars[0].LocalName)

		// Verify second env var with double colon
		assert.Equal(t, "some:id-with-double-dot", config.DynamicEnv.Vault.StructuredEnvVars[1].RemoteName)
